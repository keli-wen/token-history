...
The collector still requests `claude daily` and `codex daily` separately, with the configured dates, timezone, and `--mode auto`. Other executables must accept these arguments and emit the same per-source JSON. They must also support `--version`: exit successfully and print a version such as `my-collector 1.2.3`; the last token on the final output line is recorded. An unsupported or failed version command stops collection before snapshot writes. Removing `collector` restores `npx -y ccusage@latest` (or your existing `ccusage.spec`). A failed custom executable stops collection.
